...
已知异常测试日期（默认自动剔除，input_source=hdf5 时生效）：
  ...
  默认通过 --exclude_dates 自动剔除这些日期（不参与推理/指标统计）；
  如需保留，显式传入 --exclude_dates "" 即可关闭剔除。
...
增加输出物理约束（不包含 TMAX>=TMIN），并提供开关
约束内容（仅在 output_space=physical 生效）：
...
# 已知源数据异常日期（见文件头说明），默认在 --exclude_dates 中自动剔除。
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--exclude_dates",
        default=",".join(KNOWN_BAD_TEST_DATES),
        type=str,
        help=(
            "comma-separated YYYYMMDD dates to skip during hdf5 inference (applies to input_source=hdf5). "
            f"Default excludes known-bad hdf5_test/SON samples: {','.join(KNOWN_BAD_TEST_DATES)} "
            "(source data anomaly, see file header docstring). Pass an empty string to disable."
        ),
    )
    p.add_argument(
        "--coord_ref_nc",
    ...
    p.add_argument(
        ...
        help=(
            ...
            "pos0_360 keeps the native column ordering and writes lon 0..360; "
            "neg180_180 rolls by W//2 and writes lon -180..180."
        ),
    )
...
def _apply_lon_convention_to_pred(
    ...
    Important:
      - In this project, the effective "native" ordering used by training HDF5
        (and what the model has learned) matches lon=0..360 monotonic indexing.
      - Therefore:
          pos0_360    : keep data as-is, write lon 0..360
          neg180_180  : roll by W//2, write lon -180..180
    ...
    if lon_convention == "neg180_180":
        ...
        return pred2, _lon_axis_neg180_180(w)
    if lon_convention == "pos0_360":
        return pred, _lon_axis_pos0_360(w)
...
def main() -> None:
    ...
    exclude_dates_set: set[str] = {
        _normalize_date_yyyymmdd(d)[:8] for d in args.exclude_dates.split(",") if d.strip()
    }
    if exclude_dates_set:
        print(f"[exclude_dates] will skip {len(exclude_dates_set)} known-bad date(s): {sorted(exclude_dates_set)}")
    ...
    if input_source == "cesm_nc":
        ...
        for nc_path in cesm_files:
            ...
            x_t = torch.from_numpy(x_raw).to(device=device, dtype=torch.float32)[None, ...]  # (1,8,180,360)
            x_norm = (x_t - mean) / std

            sza_hr = torch.from_numpy(np.array(ds._cos_sza_hr(dstr), copy=True)).to(device=device, dtype=torch.float32)[None, ...]
            x_lr = x_norm   # (1, 8, 180, 360) — 仅 8 个归一化气候变量
            hr_aux = torch.cat([hr_static_t, sza_hr], dim=1)

            with torch.no_grad():
                if use_amp:
                    with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                        pred_z = model(x_lr, hr_aux)
                else:
                    pred_z = model(x_lr, hr_aux)

            pred_out = _to_output_space(pred_z, mean, std, out_space, q_output_unit, enforce_constraints)
            pred_cpu = pred_out.squeeze(0).detach().to("cpu").numpy().astype(out_np_dtype, copy=False)
            ...
            if out_mode == "per_shard":
                ...
                with h5py.File(out_path, "w") as fo:
                    ...
                    with h5py.File(h5path, "r") as fi:
                        ...
                        for j, (si, dstr) in enumerate(zip(keep_i, keep_dates)):
                            ...
                            sza_hr_np = np.array(ds._cos_sza_hr(dstr), copy=True)
                            sza_hr = torch.from_numpy(sza_hr_np).to(device=device, dtype=torch.float32)[None, ...]  # (1,1,1801,3600)

                            x_lr = x_norm                                             # (1, 8, 180, 360)
                            hr_aux = torch.cat([hr_static_t, sza_hr], dim=1)         # (1,7,1801,3600)
                ...
                with h5py.File(h5path, "r") as fi:
                    ...
                    for si, dstr in zip(keep_i, keep_dates):
                        ...
                        x_t = torch.from_numpy(x_raw).to(device=device, dtype=torch.float32)[None, ...]
                        x_norm = (x_t - mean) / std
