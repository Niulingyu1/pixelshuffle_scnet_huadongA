#!/bin/bash
# SCNet「模型训练」控制台任务的正式启动脚本（容器/vcjob，非 Slurm sbatch）。
#
# 配置与 DOWNSCALE_README.md 第 9 节、slurm/train_ddp_single_node.slurm 对齐：
...
# A800 8 卡请保持 --batch_size 1；NCCL 缓冲下不要第一步就提到 2。
#
# 架构冒烟：scripts/launch_platform_smoke_single.sh / smoke_ddp.sh
